...
## rev66
- 表示期間/詳細データ/ランキングを2カラムにせず、縦1列（フル幅）で表示するよう `.grid` のデスクトップ時レイアウトを1カラムに固定。


## rev66
- 表示上の「プロモーション」を「プロモその他」に変更（従来プロモ枠に、乗車ID空欄の金額をまとめて加算する運用に対応）。


## rev66
- payments_order の「支払い額」がマイナスの行は集計から除外（安全弁）。
